...
调研问题：AIRI 是否已经实现 Live2D 模型的动态表情、动态肢体语言，以及这些能力对 AsukaBot 的可复用价值。
...
因此，对 AsukaBot 来说，AIRI 的 Live2D 部分可以作为前端实现参考，尤其是 `stage-ui-live2d` 的包拆分、motion manager 插件链、表达式 store、口型同步和情绪到 motion 的数据流。
...
判断：AIRI 的动态肢体语言不是单点函数，而是每帧参数管线。这个结构适合 AsukaBot 复用。
...
AIRI 的 motion manager 插件链值得直接借鉴。AsukaBot 可以先实现简化版：
...
- AsukaBot 当前架构应优先把 Live2D 控制保持在前端，后端只下发结构化情绪/动作指令。
...
AsukaBot 如果要追 AIRI 的体验，建议先做“情绪到 motion + 口型 + 基础参数控制”，再做 exp3 表情系统和插件化 update 管线。
