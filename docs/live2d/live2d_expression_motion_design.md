...
在 Asuka 现有 Live2D 加载能力上，补齐可由聊天事件驱动的动态表现：
...
Asuka 当前代码量较小，建议先实现简化版控制层，再逐步演进到插件链。
...
- `asuka/api/provider/tts.py`：封装 `edge-tts` 合成。
- `asuka/routes/tts.py`：`POST /api/tts`，输入文本和可选 voice，返回 `audio/mpeg`。
- `asuka/config.py`：TTS voice/rate 配置从环境变量读取。
...
- `uv run ruff check asuka/` 通过。
- `uv run mypy asuka/` 通过。
...
- 首块阈值下调为约 18 字，后续块约 42 字；文本块入队时立即开始 TTS 合成，播放循环消费已预取的音频 Blob，以降低首块和块间延迟。
