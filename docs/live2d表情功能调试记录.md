...
| 文件 | 改动 |
|------|------|
| `asukabot/routes/ws.py` | **核心修复**：流式标签提取器分片处理（`_maybe_incomplete_tag`）；表情清单文档同步 |
| `asukabot/routes/chat.py` | `/chat` 同步接口复用提取器剥离标签 |
| `asukabot/core/agent/model.py` | LLM 人格 prompt 的表情清单收窄为 6 个并加语义说明，示例同步 |
